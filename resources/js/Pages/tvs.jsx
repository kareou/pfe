...
import Nav from "../Components/mycomponents/nav.jsx";
import Footer from "@/Components/mycomponents/footer.jsx";

function movies() {
    const [genre, setGener] = useState([]);
    const [movies, setMovies] = useState([]);
    const [type, setType] = useState(10759);
    const [start, setStart] = useState(1);
    const [ready, setReady] = useState(false);
    const [g, setG] = useState("Action & Adventure");

    useEffect(() => {
        fetchtvgenre().then((data) => {
            setGener(data);
        });
    }, []);
    ...
    useEffect(() => {
        ...
        for (let i = 1; i <= 100; i++) {
            fetchtvbygenre(type, i).then((data) => {
